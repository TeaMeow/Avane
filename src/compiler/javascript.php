...
class JavaScript extends \Avane\Avane
{
    function __construct($thisOne)
    {
        if($thisOne) parent::__construct($thisOne);

        $this->javascriptListPath = $this->compiledPath . 'javascript.json';
        $this->fileTrackingPath   = $this->compiledPath . 'javascript_file_tracking.txt';
    }
    ...
    function compile()
    {
        if(!$this->checkTime() || $this->hasNew())
        {
            foreach($this->coffee as $name => $path)
            {
                ...
                $this->coffee("coffee -p $coffeePath", $scriptPath);
            }
        }
